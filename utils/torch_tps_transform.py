...
def interpolate_grid(points, scr_w, scr_h, dest_w, dest_h):
    sparse_control_points = points.reshape(-1, 2, scr_w, scr_h)
    dense_control_points = F.interpolate(sparse_control_points, size=(dest_h, dest_w), mode='bilinear', align_corners=True)
    interpolated_points = dense_control_points.reshape(-1, 2*dest_w*dest_h)
